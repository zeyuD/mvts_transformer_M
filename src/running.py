...
def validate(val_evaluator, tensorboard_writer, config, best_metrics, best_value, epoch):
    ...
    if condition:
        ...
        utils.save_model(os.path.join(config['data_dir'], 'model_best.pth'), epoch, val_evaluator.model)
...
class SupervisedRunner(BaseRunner):
    ...
    def evaluate(self, epoch_num=None, keep_all=True):
        ...
        if self.classification:
            ...
            # prec = metrics_dict['prec_avg']
            # rec = metrics_dict['rec_avg']
            # # f1 = 2*prec*rec/(prec+rec)
            # report = self.analyzer.generate_classification_report()
            # f1_avg = report[1]
            # fnr_avg = report[2]
            # # print('F1 avg: {}'.format(f1_avg))
        ...
        if keep_all:
            return self.epoch_metrics, per_batch
            # return self.epoch_metrics, per_batch, f1_avg, fnr_avg, fpr
        else:
            return self.epoch_metrics
